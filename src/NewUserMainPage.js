...
function NewUserMainPage() {
    ...
    const typesDict = { 'Hotels': 'hotel', 'Restaurants': 'restaurent', 'NightLife': 'nightlife' }
    ...
    function getnprData(type, state) {
        axios({
            method: "GET",
            url: `/${type}/${state}/0/getNPR`,
        })
            .then((response) => {
    ...
    useEffect(() => {
        if (usState && category) {
            ...
            getnprData(typesDict[category], swappedStateNames[usState])
            getMfData(typesDict[category], swappedStateNames[usState])
            getAECFData(typesDict[category], swappedStateNames[usState])
        }
    }, [usState, category]);
        ...
        <div className="wrapper">
            <header>
                ...
                <nav className="main-nav">
                    ...
                    <DropDown className="small-nav" options={selectedStates} state={usState} setState={setUsState} label="State"></DropDown>
                </nav>
